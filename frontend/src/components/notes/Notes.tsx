...
export default function Notes({ showCompleted, sortOption }: NotesProps) {
  const { notes, loading, newNote, getNotes } = useNotesStore((state) => ({
    notes: state.notes,
  ...
  const filteredNotes = showCompleted
    ? notes
    : notes.filter((note) => !note.checked);
  ...
  return (
    <div className="flex flex-col gap-5 sm:grid xl:grid-cols-4 lg:grid-cols-3 md:grid-cols-2">
      {notes.length < 1 && (
        <div>
          <p>No notes yet.</p>
          <Link to="/create" className="hover:text-light/80">
            Click here to start adding notes!
          </Link>
        </div>
      )}
      {sortNotes(filteredNotes, sortOption).map((note) => (
        <div
          className={`p-3 bg-dark rounded-sm h-max ${newNote && newNote === note.id ? "animate-flash" : ""}`}
          key={note.id}
          id={note.id}
        >
          ...
          <div className="flex flex-col m-1">
            <p className="mb-4">
              Description: <br /> {note.description}
            </p>
          </div>
