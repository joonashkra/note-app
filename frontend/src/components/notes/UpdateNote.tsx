...
export default function UpdateNote({ noteId }: UpdateNoteProps) {
  ...
  const [newNoteDeadlineDate, setNewNoteDeadlineDate] = useState<Value>(
    new Date(),
  );
  const [noteChecked, setNoteChecked] = useState(false);
  const { updateNote, getNote, note, loading } = useNotesStore((state) => ({
  ...
  const handleUpdateNote = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const confirmUpdate = window.confirm(
      "Do you wish to apply these changes permanently?",
    );
    if (confirmUpdate) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Calendar
          onChange={setNewNoteDeadlineDate}
          value={newNoteDeadlineDate}
          minDate={new Date()}
        />
      </div>
      <div
        title="Set Checked"
        className="flex flex-row items-center gap-2 lg:w-1/3 md:w-1/2 sm:w-full"
      >
        <p>Mark as Done:</p>
      ...
      <div className="flex flex-row justify-between items-center lg:w-1/3 md:w-1/2 sm:w-full">
        <button
          title="Save Changes"
          className="hover:text-light hover:border-light focus:border-light bg-dark rounded-md w-max flex gap-2"
          type="submit"
        >
          Save
        </button>
        <div className="flex flex-row gap-2">
          ...
          <Link to="/">
            <button
              title="Cancel"
              className="text-white hover:text-red hover:border-red focus:border-red bg-dark rounded-md"
              type="button"
            >
              Cancel
            </button>
