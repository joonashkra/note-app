...
export default function FilterNotes({ displayFilterOptions, sortOption, handleSortOption, showCompleted, handleShowCompleted }: FilterNotesProps) {
  return (
    <div className={`flex flex-col bg-dark p-2 rounded-lg sm:flex-row sm:w-max sm:gap-4 ${displayFilterOptions && "hidden sm:flex"}`}>
            <select value={sortOption} onChange={handleSortOption} className="bg-dark/0 p-2 w-full">
