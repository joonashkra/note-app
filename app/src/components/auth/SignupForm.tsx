...
export default function SignupForm() {
  ...
  const { signup, loggedIn } = useAuthStore((state) => ({ signup: state.signup, loggedIn: state.loggedIn }))
  ...
  return (
    <form className='flex flex-col gap-5 w-full xl:w-1/5 lg:w-1/4 md:w-1/3 sm:w-1/2' onSubmit={handleSignup}>
        ...
        <input min={6} type="password" onChange={(e) => setPassword(e.target.value)} className="p-2 mb-1 rounded-sm shadow-sm shadow-dark bg-dark" placeholder="New Password..." required/>
